...
+ (void)getImage:(ForgeTask*)task source:(NSString*)source {
	file_PictureTakerDelegate *delegate = [[file_PictureTakerDelegate alloc] initWithTask:task];
	
	dispatch_async(dispatch_get_main_queue(), ^{
		[[IKPictureTaker pictureTaker] beginPictureTakerSheetForWindow:[[[ForgeApp sharedApp] appDelegate] window] withDelegate:delegate didEndSelector:@selector(pictureTakerDidEnd:returnCode:contextInfo:) contextInfo:nil];
	});
}
...
+ (void)saveURL:(ForgeTask*)task url:(NSString*)url {
	NSString *tempFile = [[[NSFileManager defaultManager] applicationSupportDirectory] stringByAppendingPathComponent:[NSString stringWithFormat: @"saveURL-%.0f", [NSDate timeIntervalSinceReferenceDate] * 1000.0]];
